...
export const MapPage = () => {
  ...
  return (
    <>
      {/* 전체화면 지도 배경 — AppLayout 의 padding 을 음수 마진으로 상쇄 */}
      <div className="-mx-5 -mt-6 sm:-mx-6 lg:-mx-8" style={{ height: 'calc(100dvh - 120px)' }}>
        <TourMap
          geojson={geojson}
      ...
      {/* 오버레이: 좌측 관광지 목록 패널 */}
      <div
        className={`fixed z-20 transition-transform duration-300 ${
          panelOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
        style={{ top: 128, left: 0, bottom: 72, width: 340, maxWidth: 'calc(100vw - 56px)' }}
      >
        <div className="flex h-full flex-col rounded-r-2xl border border-l-0 border-line bg-white/95 shadow-xl backdrop-blur-md">
          {/* 패널 헤더 */}
          <div className="flex items-center justify-between border-b border-line px-4 py-3">
            <h2 className="text-[14px] font-bold text-ink">
              {loading ? '불러오는 중...' : `관광지 ${visiblePlaces.length}개`}
            </h2>
            <button
              type="button"
              onClick={() => setPanelOpen(false)}
              className="grid h-7 w-7 place-items-center rounded-md text-ink-3 hover:bg-neutral-100"
            >
          ...
          <div className="flex-1 overflow-y-auto">
            {visiblePlaces.map((p) => (
              <button
                ...
                className="flex w-full items-center gap-3 border-b border-line/60 px-4 py-3 text-left transition-colors hover:bg-neutral-50 active:bg-neutral-100"
              >
                {p.thumbnail ? (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-[11px] text-ink-3">{p.addr}</p>
                </div>
              </button>
            ))}
            {!loading && visiblePlaces.length === 0 && (
              <p className="px-4 py-12 text-center text-[13px] text-ink-3">
                지도를 이동하면 관광지가 표시됩니다
              </p>
            )}
          </div>
      ...
      {!panelOpen && (
        <button
          ...
          className="fixed z-20 flex items-center gap-2 rounded-r-2xl border border-l-0 border-line bg-white/95 px-3 py-2.5 shadow-lg backdrop-blur-sm"
        ...
        >
          ...
          <span className="text-[12px] font-bold text-ink-2">목록</span>
        </button>
      )}
    </>
  )
